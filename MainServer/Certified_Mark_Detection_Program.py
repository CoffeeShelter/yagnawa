...
def calculating_dszie(image, value, isWidth):
    index = 0
    index2 = 1
    if isWidth:
        index = 1
    ...
    aspect_ratio = float(value) / image.shape[index]
    dsize = (value, int(image.shape[index2] * aspect_ratio))

    return dsize


def resizing_image(target, isWidth=True):
    dsize = calculating_dszie(target, 1400, isWidth)
    resized = cv2.resize(target, dsize, interpolation=cv2.INTER_AREA)
    return resized
